...
const mocks = vi.hoisted(() => ({
  ...
  zonesRepository: {
    ...
    findCoverageByDistributor: vi.fn(),
...
/** Zona base usada nos testes — cobertura Centro + 36010-000. */
function zoneFixture(overrides: Record<string, unknown> = {}) {
  return {
    ...
    is_active: true,
    coverage: [
      { id: "cov-1", neighborhood: "Centro", zip_code: "36010-000" },
    ],
    ...overrides,
  };
}
...
beforeEach(() => {
  ...
  mocks.zonesRepository.findCoverageByDistributor.mockResolvedValue([]);
...
describe("zonesService.addCoverageBulk", () => {
    ...
    mocks.zonesRepository.findCoverageByDistributor.mockResolvedValue([
      {
        ...
        neighborhood: "Centro",
        zip_code: null,
        zone: { id: OTHER_ZONE_ID, name: "JF — Norte" },
      },
    ]);
    ...
    mocks.zonesRepository.findCoverageByDistributor.mockResolvedValue([
      {
        ...
        zip_code: null,
        zone: { id: OTHER_ZONE_ID, name: "JF — Sul" },
      },
    ]);
  ...
  it("imports the non-conflicting entries and skips only the conflicting ones", async () => {
    ...
    expect(mocks.zonesRepository.createCoverage).toHaveBeenCalledWith(
      expect.objectContaining({ neighborhood: "Granbery" }),
      expect.anything()
    );
  });
    ...
    mocks.zonesRepository.findCoverageByDistributor.mockResolvedValue([
      {
        ...
        neighborhood: "Centro",
        zip_code: null,
        zone: { id: "zone-destino", name: "Centro AF" },
      },
    ]);
