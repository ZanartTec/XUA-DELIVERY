...
export const subscriptionPlansController = {
  ...
  async getOne(req: Request, res: Response, next: NextFunction): Promise<void> {
    try {
      const plan = await subscriptionPlansService.getPlan(req.params.id as string);
      res.json(plan);
    } catch (err) {
  ...
  async update(req: Request, res: Response, next: NextFunction): Promise<void> {
    ...
    try {
      const plan = await subscriptionPlansService.updatePlan(req.params.id as string, parsed.data);
      res.json({ plan });
    } catch (err) {
      if (err instanceof Error && err.message === "PLAN_NOT_FOUND") {
        res.status(404).json({ error: "Plano não encontrado" });
