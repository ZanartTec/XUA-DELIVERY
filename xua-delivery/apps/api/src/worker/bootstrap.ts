...
/**
 * Cria os workers BullMQ, registra os job schedulers e instala o graceful
 * shutdown. Separado de index.ts para que a validação de ambiente rode ANTES
 * deste módulo ser importado — vários imports daqui validam env no topo do
 * arquivo e derrubariam o processo no primeiro erro, em vez de listar todos.
 */
export function startWorker(): void {
  ...
  const internalJobsWorker = new Worker<InternalJobPayload>(
    ...
    processInternalJob,
    {
      ...
      prefix: QUEUE_PREFIX,
      concurrency: env.INTERNAL_JOBS_WORKER_CONCURRENCY ?? 1,
    }
  );
  ...
  const paymentWebhooksWorker = new Worker<PaymentWebhookJobPayload>(
    ...
    processPaymentJob,
    {
      connection: createQueueRedisConnection(),
      prefix: QUEUE_PREFIX,
      concurrency: env.PAYMENT_WEBHOOK_WORKER_CONCURRENCY ?? 1,
    }
  );
  ...
  const paymentsWorker = new Worker<PaymentExpirationJobPayload>(
    ...
    processPaymentExpiration,
    {
      connection: createQueueRedisConnection(),
      prefix: QUEUE_PREFIX,
      concurrency: env.PAYMENTS_WORKER_CONCURRENCY ?? 1,
    }
  );
  ...
  const paymentRefundsWorker = new Worker<PaymentRefundJobPayload>(
    ...
    processPaymentRefund,
    {
      connection: createQueueRedisConnection(),
      prefix: QUEUE_PREFIX,
      concurrency: env.PAYMENT_REFUNDS_WORKER_CONCURRENCY ?? 1,
    }
  );
  ...
  const subscriptionExpirationWorker = new Worker<SubscriptionExpirationJobPayload>(
    ...
    processSubscriptionExpiration,
    {
      connection: createQueueRedisConnection(),
      prefix: QUEUE_PREFIX,
      concurrency: env.SUBSCRIPTION_EXPIRATION_WORKER_CONCURRENCY ?? 1,
    }
  );
