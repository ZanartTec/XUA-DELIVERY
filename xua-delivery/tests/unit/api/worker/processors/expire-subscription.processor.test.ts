...
vi.mock("@api/infra/prisma/client", () => ({ getPrisma: () => mocks.prisma }));
vi.mock("@api/infra/logger", () => ({ createLogger: () => mocks.logger }));
vi.mock("@api/modules/audit/audit.repository.js", () => ({
  auditRepository: mocks.auditRepository,
}));

const { processSubscriptionExpiration } = await import("@api/worker/processors/expire-subscription.processor.js");
