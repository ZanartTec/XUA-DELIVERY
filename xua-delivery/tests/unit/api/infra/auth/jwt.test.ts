...
const { signToken, verifyToken } = await import("@api/infra/auth/jwt.js");
...
describe("validação de JWT_SECRET no boot (module load)", () => {
  it("recusa subir sem JWT_SECRET definido", async () => {
    ...
    vi.resetModules();
    await expect(import("@api/infra/auth/jwt.js")).rejects.toThrow(/JWT_SECRET não definido/);
  ...
  it("recusa segredo com menos de 32 caracteres", async () => {
    ...
    vi.resetModules();
    await expect(import("@api/infra/auth/jwt.js")).rejects.toThrow(/pelo menos 32 caracteres/);
    process.env.JWT_SECRET = original;
  });
  ...
  it("recusa valor placeholder do template de exemplo", async () => {
    ...
    vi.resetModules();
    await expect(import("@api/infra/auth/jwt.js")).rejects.toThrow(/placeholder/);
    process.env.JWT_SECRET = original;
  });
});
