...
vi.mock("@api/modules/auth/repository/auth.repository.js", () => ({
  authRepository: mocks.authRepository,
}));
vi.mock("@api/modules/auth/repository/password-reset.repository.js", () => ({
  passwordResetRepository: mocks.passwordResetRepository,
}));
vi.mock("@api/infra/mail/mailer.js", () => ({ sendMail: mocks.sendMail }));
vi.mock("@api/infra/auth/password.js", () => ({ hashPassword: mocks.hashPassword }));
vi.mock("@api/infra/auth/password-change.js", () => ({
  markPasswordChanged: mocks.markPasswordChanged,
}));
vi.mock("@api/infra/prisma/client.js", () => ({
  getPrisma: () => ({ $transaction: mocks.transaction }),
}));
vi.mock("@api/modules/auth/services/auth.service.js", () => ({
...
const { passwordResetService } = await import("@api/modules/auth/services/password-reset.service.js");
