...
| 19 | **Rate limiting era opt-in por rota sem política declarada (🟡 parcialmente resolvido 09/08/2026)** | Auditoria de segurança+arquitetura (09/08/2026) encontrou: `RATE_LIMITS.global` (100/min) configurado mas nunca referenciado em código nenhum — a doc antiga (`guia-tecnico.md`) descrevia uma proteção que nunca existiu em runtime; `RATE_LIMITS.auth` (10/min) também configurado e nunca ligado, deixando `POST /api/auth/login` e `/register` — rotas públicas, sem auth — sem qualquer throttle (achado crítico: força bruta/credential stuffing). **Resolvido nesta entrega**: `RATE_LIMITS` reestruturado em categorias reutilizáveis (`publicRead`, `authenticatedRead`, `authenticatedWrite`, `sensitiveAction`, `bulkImport`, `heavyRead`, `externalLookup`) em `apps/api/src/infra/rate-limit/limiter.ts`; `global` (morto) removido; `auth` ligado em login/register (chaveado por IP+e-mail); aplicado nos módulos que não tinham nenhuma rota protegida: `ops` (incl. `/audit/export`, antes sem limite apesar de ser export pesado), `distributor`, `consumers` (CEP lookup com categoria própria por depender de serviço externo), `driver`, `subscription-plans`, `user-subscriptions`. **Ainda não coberto** (risco baixo, não bloqueante): `notifications` (`POST /subscribe`), `distributors-public` (leitura pública de baixo custo). **Débito de observability, não implementado**: o fail-open do Redis (`rate-limit.ts` — se Redis cair, a rota fica sem limite silenciosamente, só loga `warn`) é aceitável para leitura, mas para `paymentCharge`/`auth` deveria promover a `logger.error` com campo estruturado assim que houver alerting real (Sentry/Prometheus, hoje inexistente) — recomendação da arquitetura, não implementada por depender de infra que ainda não existe |
