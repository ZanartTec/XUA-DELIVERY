...
| Dev A — Backend / Services | Dev B — Frontend / UI |
|---|---|
| Rate limiting: categorias reutilizáveis em `RATE_LIMITS` (`apps/api/src/infra/rate-limit/limiter.ts`) — `publicRead`/`authenticatedRead`/`authenticatedWrite`/`sensitiveAction`/`bulkImport`/`heavyRead`/`externalLookup` — mais overrides nomeados por domínio (10/min auth login/register, 30/min orders, etc). [ESTADO ATUAL — 09/08/2026: não existe mais um limite "global" único — cada módulo aplica sua categoria explicitamente; auditoria completa em `04-active-state.md`] | Página `/subscription/create`: wizard 5 etapas (Plano → Distribuidor → Endereço → Datas → Pagamento) |
